...
class TimeTableCtl(BaseCtl):
    ...
    def submit(self, request, params={}):
        if params['id'] > 0:
            dup = self.get_service().get_model().objects.exclude(id=params['id']).filter(course_ID=self.form["course_ID"], subject_ID=self.form["subject_ID"], examDate=self.form["examDate"], examTime= self.form['examTime'], semester = self.form["semester"])
            if dup.count() > 0:
                self.form["error"] = True
            ...
            duplicity = self.get_service().Duplicate(self.form["course_ID"], self.form["subject_ID"],self.form["examDate"], self.form['examTime'], self.form["semester"])
